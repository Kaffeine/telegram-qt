    ...
    m_appState(AppStateNone)
{
    ...
    connect(m_backend, SIGNAL(authSignErrorReceived(Telegram::Namespace::AuthSignError,QString)),
            SLOT(onAuthSignErrorReceived(Telegram::Namespace::AuthSignError,QString)));
    ...
    connect(m_backend, SIGNAL(contactChatMessageActionChanged(quint32,quint32,Telegram::Namespace::MessageAction)),
            SLOT(onContactChatMessageActionChanged(quint32,quint32,Telegram::Namespace::MessageAction)));
    connect(m_backend, SIGNAL(contactMessageActionChanged(quint32,Telegram::Namespace::MessageAction)),
            SLOT(onContactMessageActionChanged(quint32,Telegram::Namespace::MessageAction)));
...
void MainWindow::onAuthSignErrorReceived(Telegram::Namespace::AuthenticationError errorCode, const QString &errorMessage)
{
    switch (errorCode) {
...
void MainWindow::onMessageReceived(const Telegram::Peer peer, quint32 messageId)
{
    ...
    if (groupChatMessage) {
        ...
        if (!(message.flags & Telegram::Namespace::MessageFlagOut) && (m_contactLastMessageList.value(peer.id) < peer.id)) {
...
void MainWindow::initStartConnection()
{
    ...
    if (ui->settingsReceivingFilterReadMessages->isChecked()) {
        flags |= Telegram::Namespace::MessageFlagRead;
    }
