...
public:
    ...
    PendingMessagesAffectedMessages *deleteMessages(const TLInputChannel &channel, const QVector<quint32> &id);
    ...
    PendingChannelsAdminLogResults *getAdminLog(quint32 flags, const TLInputChannel &channel, const QString &q, const TLChannelAdminLogEventsFilter &eventsFilter, const QVector<TLInputUser> &admins, quint64 maxId, quint64 minId, quint32 limit);
    PendingMessagesChats *getAdminedPublicChannels();
    PendingMessagesChats *getChannels(const QVector<TLInputChannel> &id);
    PendingMessagesChatFull *getFullChannel(const TLInputChannel &channel);
    PendingMessagesChats *getLeftChannels(quint32 offset);
    PendingMessagesMessages *getMessages(const TLInputChannel &channel, const QVector<TLInputMessage> &id);
    ...
    PendingUpdates *inviteToChannel(const TLInputChannel &channel, const QVector<TLInputUser> &users);
    ...
    PendingBool *readMessageContents(const TLInputChannel &channel, const QVector<quint32> &id);
    PendingBool *reportSpam(const TLInputChannel &channel, const TLInputUser &userId, const QVector<quint32> &id);
